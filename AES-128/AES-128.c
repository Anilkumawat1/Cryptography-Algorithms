...
//this is key scheduling algorithm generating the 11 round key
 void keySchedulingAES(unsigned char key[16]){
uint32_t  Rconstant[10];
uint32_t  word[44];
...
Rconstant[8] = 0x1B000000;
Rconstant[9] = 0x36000000;
...
uint32_t w = word[i-1];
if(i%4==0){
w = subword((w<<8) | (w >> 24)) ^ Rconstant[i/4 - 1];
}
word[i] = word[i-4]^w;
}
int k = 0;
...
unsigned char polynomial_multiply(unsigned char c, int i){
for(int j=0; j<i; j++){
...
for(int i=0;i<4;i++){
for(int j=0;j<4;j++){
x[j][i] = mix[j][i] ^ polynomial_multiply(mix[(j+1)%4][i],2) ^ polynomial_multiply(mix[(j+2)%4][i],2) ^ polynomial_multiply(mix[(j+3)%4][i],2) ^ mix[(j+3)%4][i];
}
}
}
...
for(int i=0;i<4;i++){
for(int j=0;j<4;j++){
x[j][i] = mix[j][i] ^ polynomial_multiply(mix[j][i],7)^ polynomial_multiply(mix[j][i],5)^ polynomial_multiply(mix[j][i],2)^mix[(j+1)%4][i]^ polynomial_multiply(mix[(j+1)%4][i],1)^ polynomial_multiply(mix[(j+1)%4][i],2)  ^ polynomial_multiply(mix[(j+2)%4][i],1)^ polynomial_multiply(mix[(j+2)%4][i],3)^ polynomial_multiply(mix[(j+2)%4][i],4) ^ polynomial_multiply(mix[(j+3)%4][i],1)^ polynomial_multiply(mix[(j+3)%4][i],4)^ polynomial_multiply(mix[(j+3)%4][i],5)^ polynomial_multiply(mix[(j+3)%4][i],6) ^ mix[(j+3)%4][i];
}
}
}
...
x = (x<<4)|j;
 x^=1;
 x = polynomial_multiply(x, 7)^polynomial_multiply(x, 3)^polynomial_multiply(x, 2)^x;
...
for(int i=0;i<4;i++){
for(int j=0;j<4;j++){
plaintext[i][j] = subbytes(polynomial_multiply(plaintext[i][j], 1)^1);
}
}
}
...
for(int i=0;i<4;i++){
for(int j=0;j<4;j++){
plaintext[i][j] ^=  roundKey[round][k];
k++;
}
}
}
...
int main(){
  unsigned char plaintext[4][4];//= {{0x7c,0xad,0x80,0x82},{0x7c,0x26,0x30,0xad},{0x7c,0xe5,0x80,0x26},{0x7c,0x82,0x30,0xe5}};
  ...
  printf("Enter the plaintext of 128-bit : ");
  for(int i = 0;i<4;i++){
  for(int j=0;j<4;j++){
  ...
  printf("Enter the key of 128-bit : ");
  for(int i = 0;i<16;i++){
 scanf("%hhx",&key[i]);
   }
  //printing the key
